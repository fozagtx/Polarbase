...
function assertIsValidBuffer(methodName, bufferToCheck, context) {
  (0, import_sdk_errors9.assert)(
    ...
    import_sdk_errors9.RLP.INVALID_RLP,
...
var _unpackData = (packed, profile, context) => {
  ...
  if (kind instanceof RLP2.ScalarKind) {
    (0, import_sdk_errors10.assert)(
      ...
      import_sdk_errors10.RLP.INVALID_RLP,
...
var validateNumericKindData = (data, context) => {
  (0, import_sdk_errors11.assert)(
    ...
    import_sdk_errors11.RLP.INVALID_RLP,
...
var assertValidHexBlobKindData = (data, context) => {
  (0, import_sdk_errors12.assert)(
    ...
    typeof data === "string",
    import_sdk_errors12.RLP.INVALID_RLP,
...
var assertFixedHexBlobKindData = (data, context, bytes4) => {
  (0, import_sdk_errors13.assert)(
    "assertFixedHexBlobKindData",
    data.length === bytes4 * 2 + 2,
    import_sdk_errors13.RLP.INVALID_RLP,
    `Validation error: Hex string in ${context} must be exactly ${bytes4} bytes in length.`,
    { data, context }
  );
};
var assertFixedHexBlobKindBuffer = (buffer, context, bytes4) => {
  (0, import_sdk_errors13.assert)(
    "assertFixedHexBlobKindBuffer",
    buffer.length === bytes4,
    import_sdk_errors13.RLP.INVALID_RLP,
    `Validation error: Buffer in ${context} must be exactly ${bytes4} bytes in length.`,
    { buffer, context }
  );
};
...
var assertCompactFixedHexBlobBuffer = (buffer, context, bytes4) => {
  (0, import_sdk_errors14.assert)(
    "assertCompactFixedHexBlobBuffer",
    buffer.length <= bytes4,
    import_sdk_errors14.RLP.INVALID_RLP,
    `Validation error: Buffer in ${context} must be at most ${bytes4} bytes.`,
    { buffer, context }
  );
  (0, import_sdk_errors14.assert)(
    "assertCompactFixedHexBlobBuffer",
...
var decodeBufferToHexWithLeadingZeros = (buffer, bytes4) => {
  return Hex0x.of(buffer, bytes4);
};
...
function ofBigInt(bi, bytes4) {
  ...
  return pad(bi.toString(RADIX), bytes4);
}
function ofHexString(n, bytes4) {
  (0, import_sdk_errors15.assert)(
    "hex.ts.ofHexString",
    Hex0x.isValid(n),
    import_sdk_errors15.DATA.INVALID_DATA_TYPE,
  ...
  return pad(n.slice(2).toLowerCase(), bytes4);
}
function ofNumber(n, bytes4) {
  (0, import_sdk_errors15.assert)(
    "hex.ts.ofNumber",
    Number.isInteger(n),
    import_sdk_errors15.DATA.INVALID_DATA_TYPE,
  ...
  return pad(n.toString(RADIX), bytes4);
}
function ofString(txt, bytes4) {
  return ofUint8Array(utf8ToBytes(txt), bytes4);
}
function ofUint8Array(uint8Array, bytes4) {
  return pad(bytesToHex(uint8Array), bytes4);
}
function pad(exp, bytes4) {
  let result = exp;
  if (result.length % 2 !== 0) {
    result = "0" + result;
  }
  if (bytes4 > 0) {
    const gap = bytes4 - result.length / 2;
    if (gap > 0) {
...
var Hex0x = {
   ...
   */
  canon: function(exp, bytes4) {
    return `${PREFIX}${Hex.canon(exp, bytes4)}`;
  },
  /**
   ...
   */
  of: function(n, bytes4 = 0) {
    return `${PREFIX}${Hex.of(n, bytes4)}`;
  }
};
var Hex = {
  /**
  ...
  canon: function(exp, bytes4) {
    ...
    if (typeof bytes4 !== "undefined") {
      (0, import_sdk_errors15.assert)(
        "Hex.canon",
        Number.isInteger(bytes4) && bytes4 >= 0,
        import_sdk_errors15.DATA.INVALID_DATA_TYPE,
        "Arg 'bytes' not a length." /* NOT_LENGTH */,
        { bytes: bytes4 }
      );
      result = pad(result, bytes4);
      (0, import_sdk_errors15.assert)(
        "Hex.canon",
        result.length <= bytes4 * 2,
        import_sdk_errors15.DATA.INVALID_DATA_TYPE,
  ...
  of: function(n, bytes4 = 0) {
    if (n instanceof Uint8Array)
      return ofUint8Array(n, bytes4);
    if (typeof n === "bigint")
      return ofBigInt(n, bytes4);
    if (typeof n === `number`)
      return ofNumber(n, bytes4);
    if (Hex0x.isValid(n))
      return ofHexString(n, bytes4);
    return ofString(n, bytes4);
  },
  /**
   ...
   */
  random: function(bytes4) {
    return ofUint8Array(randomBytes(bytes4), bytes4);
  }
};
var Quantity = {
  /**
...
function blake2b256(data, returnType = "buffer") {
  ...
  const hash4 = _blake2b256(...dataBytesLike);
...
var addressesOf = (block) => {
  ...
  block.transactions.forEach(
    (transaction) => {
      ...
      transaction.outputs.forEach((output4) => {
        if (typeof output4.contractAddress === "string") {
          addresses.add(output4.contractAddress);
        }
        output4.events.forEach((event) => {
          addresses.add(event.address);
        });
        output4.transfers.forEach((transfer) => {
...
var isInBloom = (filter, k, data) => {
  (0, import_sdk_errors16.assert)(
    ...
    isBloom(filter),
    import_sdk_errors16.BLOOM.INVALID_BLOOM,
  ...
  (0, import_sdk_errors16.assert)(
    ...
    import_sdk_errors16.BLOOM.INVALID_K,
...
var isAddressInBloom = (filter, k, address) => {
  (0, import_sdk_errors16.assert)(
    ...
    addressUtils.isAddress(address),
    import_sdk_errors16.ADDRESS.INVALID_ADDRESS,
...
import_bignumber.BigNumber.set({ DECIMAL_PLACES: BIG_NUMBER_PRECISION });
  ...
  switch (typeof value) {
    ...
    case "string": {
      if (Hex0x.isValid(value)) {
        bn = (0, import_bignumber.BigNumber)(hexToNumber(Hex.canon(value)).toString());
      } else {
        bn = (0, import_bignumber.BigNumber)(value);
      }
    }
...
var isRevisionAccount = (revision) => {
  return revision === "best" || typeof revision === "string" && Hex0x.isValid(revision) || typeof revision === "string" && dataUtils.isDecimalString(revision) || typeof revision === "number" && revision >= 0;
};
...
function recover(messageHash, sig) {
  ...
  (0, import_sdk_errors20.assert)(
    ...
    import_sdk_errors20.SECP256K1.INVALID_SECP256k1_SIGNATURE,
  ...
  (0, import_sdk_errors20.assert)(
    ...
    import_sdk_errors20.SECP256K1.INVALID_SECP256k1_SIGNATURE_RECOVERY,
...
function toERC55Checksum(address) {
  (0, import_sdk_errors21.assert)(
    ...
    isAddress(address),
    import_sdk_errors21.ADDRESS.INVALID_ADDRESS,
...
function verify(cert) {
  (0, import_sdk_errors22.assert)(
    ...
    import_sdk_errors22.CERTIFICATE.CERTIFICATE_NOT_SIGNED,
  ...
  (0, import_sdk_errors22.assert)(
    ...
    import_sdk_errors22.CERTIFICATE.CERTIFICATE_INVALID_SIGNATURE_FORMAT,
  ...
  try {
    ...
    const pubKey = secp256k12.recover(
      signingHash,
      hexToBytes(Hex.canon(cert.signature))
    );
    (0, import_sdk_errors22.assert)(
      "certificate.verify",
      addressUtils.fromPublicKey(pubKey).toLowerCase() === cert.signer.toLowerCase(),
      import_sdk_errors22.CERTIFICATE.CERTIFICATE_INVALID_SIGNER,
...
function functionInteraction(contractAddress, functionFragment, args, value = 0) {
  return {
    to: contractAddress,
    value,
...
function transferToken(tokenAddress, recipientAddress, amount) {
  try {
    ...
    throw (0, import_sdk_errors23.buildError)(
      "transferToken",
      import_sdk_errors23.DATA.INVALID_DATA_TYPE,
...
var bip32 = __toESM(require("@scure/bip32"));
...
// src/hdnode/hdnode.ts
var X_PRIV_PREFIX2 = hexToBytes("0488ade4000000000000000000");
var X_PUB_PREFIX2 = hexToBytes("0488b21e000000000000000000");
function fromMnemonic(words, path = VET_DERIVATION_PATH) {
  ...
  try {
    ...
    throw (0, import_sdk_errors24.buildError)(
      "HDNode.fromMnemonic",
      import_sdk_errors24.HDNODE.INVALID_HDNODE_MNEMONICS,
  ...
  try {
    ...
    throw (0, import_sdk_errors24.buildError)(
      "HDNode.fromMnemonic",
      import_sdk_errors24.HDNODE.INVALID_HDNODE_DERIVATION_PATH,
      error.message,
      { path },
      error
    );
...
function fromPrivateKey2(privateKey, chainCode) {
  if (privateKey.length === 32) {
    ...
    try {
      ...
      throw (0, import_sdk_errors24.buildError)(
        ...
        import_sdk_errors24.HDNODE.INVALID_HDNODE_CHAIN_CODE,
        error.message,
        { chainCode }
      );
    }
  ...
  throw (0, import_sdk_errors24.buildError)(
    "HDNode.fromPrivateKey",
    import_sdk_errors24.HDNODE.INVALID_HDNODE_PRIVATE_KEY,
  ...
  if (chainCode.length === 32) {
    ...
    try {
      ...
      throw (0, import_sdk_errors24.buildError)(
        ...
        import_sdk_errors24.HDNODE.INVALID_HDNODE_PUBLIC_KEY,
        "Invalid public key.",
        { publicKey },
        error
      );
...
async function decrypt(keystore4, password) {
  (0, import_sdk_errors25.assert)(
    "keystore.decrypt",
    isValid(keystore4),
    import_sdk_errors25.KEYSTORE.INVALID_KEYSTORE,
  ...
  try {
    return await import_ethers4.ethers.decryptKeystoreJson(
      JSON.stringify(keystore4),
      password
    );
  } catch (e) {
    throw (0, import_sdk_errors25.buildError)(
      "keystore.decrypt",
      import_sdk_errors25.KEYSTORE.INVALID_PASSWORD,
...
function isValid(keystore4) {
  return import_ethers4.ethers.isKeystoreJson(JSON.stringify(keystore4));
}
...
function decodeScryptParams(keystore4) {
  ...
  (0, import_sdk_errors26.assert)(
    "keystore.decrypt",
    N > 0 && (N & N - 1) === 0,
    import_sdk_errors26.KEYSTORE.INVALID_KEYSTORE,
...
function encrypt3(privateKey, password) {
  ...
  try {
    ...
    return {
      ...
      crypto: {
        ...
        mac: Hex.of(keccak256(concatBytes(macPrefix, ciphertext)))
      },
...
function decrypt3(keystore4, password) {
...
function decryptKeystore(keystore4, password) {
  try {
    ...
    (0, import_sdk_errors26.assert)(
      ...
      import_sdk_errors26.KEYSTORE.INVALID_PASSWORD,
      "Decryption failed: invalid password for the given keystore."
    );
...
function isValid2(keystore4) {
  try {
    const copy = JSON.parse(JSON.stringify(keystore4));
...
function uuidV4(bytes4) {
  bytes4[6] = bytes4[6] & 15 | 64;
  bytes4[8] = bytes4[8] & 63 | 128;
  const value = Hex.of(bytes4);
  return [
    value.substring(0, 8),
...
var keystore2 = { decrypt: decrypt3, encrypt: encrypt3, isValid: isValid2 };
...
async function encrypt4(privateKey, password) {
  ...
  return EXPERIMENTAL_CRYPTOGRAPHY ? keystore2.encrypt(
    privateKey,
    new TextEncoder().encode(password.normalize("NFKC"))
  ) : await keystore.encrypt(privateKey, password);
}
async function decrypt4(keystore4, password) {
  if (EXPERIMENTAL_CRYPTOGRAPHY)
    (0, import_sdk_logging.VechainSDKLogger)("warning").log({
      title: `Experimental cryptography`,
      messages: [
  ...
  return EXPERIMENTAL_CRYPTOGRAPHY ? keystore2.decrypt(
    keystore4,
    new TextEncoder().encode(password.normalize("NFKC"))
  ) : await keystore.decrypt(keystore4, password);
}
...
var keystore3 = { encrypt: encrypt4, decrypt: decrypt4, isValid: isValid3, useExperimentalCryptography };

// src/mnemonic/mnemonic.ts
var bip392 = __toESM(require("@scure/bip39"));
...
function generate(wordlistSize, randomGenerator) {
  ...
  return bip392.entropyToMnemonic(
    randomGenerator(
...
function isValid4(words) {
  return bip392.validateMnemonic(words.join(" "), import_english.wordlist);
}
var mnemonic = {
  deriveAddress,
  derivePrivateKey,
...
var Transaction = class _Transaction {
  ...
  constructor(body, signature) {
    (0, import_sdk_errors28.assert)(
      ...
      import_sdk_errors28.TRANSACTION.INVALID_TRANSACTION_BODY,
    ...
    if (signature !== void 0) {
      (0, import_sdk_errors28.assert)(
        ...
        import_sdk_errors28.SECP256K1.INVALID_SECP256k1_SIGNATURE,
  ...
  get delegator() {
    (0, import_sdk_errors28.assert)(
      "delegator",
      this.isDelegated,
      import_sdk_errors28.TRANSACTION.INVALID_DELEGATION,
  ...
  getSignatureHash(delegateFor) {
    (0, import_sdk_errors28.assert)(
      ...
      import_sdk_errors28.ADDRESS.INVALID_ADDRESS,
...
function sign2(transactionBody, signerPrivateKey) {
  ...
  (0, import_sdk_errors29.assert)(
    ...
    import_sdk_errors29.TRANSACTION.INVALID_DELEGATION,
...
function _decodeReservedField(reserved) {
  (0, import_sdk_errors30.assert)(
    ...
    import_sdk_errors30.TRANSACTION.INVALID_TRANSACTION_BODY,
