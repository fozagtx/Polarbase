...
async function checkContractsDeployment() {
  try {
    const marketplaceContract = config.marketplaceContractAddress;
    const code = marketplaceContract
      ? await ethers.provider.getCode(marketplaceContract)
      : "0x";
    if (code === "0x") {
      console.log(
        `marketplace contract not deployed at address ${config.marketplaceContractAddress}`,
      );
      if (isSoloNetwork) {
    ...
    } else console.log(`marketplace contract already deployed`);
  } catch (e) {
    console.log(e);
  }
}
...
async function overrideLocalConfigWithNewContracts(
  ...
  const newConfig: AppConfig = {
    ...config,
    marketplaceContractAddress: await contracts.marketplace.getAddress(),
  };
