...
export async function deployAll(config: ContractsConfig) {
  ...
  // Deploy the DigitalProductStore contract
  const contractName = "DigitalProductStore";
  const DigitalProductStoreContract =
    await ethers.getContractFactory(contractName);
  const marketplace = await DigitalProductStoreContract.deploy();
  await marketplace.waitForDeployment();
  console.log(`${contractName} impl.: ${await marketplace.getAddress()}`);
